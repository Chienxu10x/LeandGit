...
class AppWidget2 : AppWidgetProvider() {
    ...
    appWidgetId : Int){
        val views = RemoteViews(context.packageName, R.layout.widget_icon_2)
        val currentWidget = dataRepository.getCurrentWidget()
        val bitmap = context.getBitmapFromAsset("Widget/widget$currentWidget/widget_4.webp")
    ...
    override fun onUpdate(
        ...
        if(appWidgetIds!!.isNotEmpty()){
            context?.let { updateAppWidget(it, appWidgetManager!!, appWidgetIds[appWidgetIds.size - 1]) }
        }
